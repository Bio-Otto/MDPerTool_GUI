...
from PySide2.QtWidgets import QApplication, QMainWindow, QGraphicsDropShadowEffect
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        UIF.labelTitle(self, 'MDPerTool - %s %s' % (platform.system(), platform.release()))
        UIF.labelDescription(self, str(os.getcwd()))
        ...
        # --------------------------------------------- > CREATE MENUS < --------------------------------------------- #
        # ----- > Toggle Menu Size
        self.btn_toggle_menu.clicked.connect(lambda: UIF.toggleMenu(self, 220, True))

        # ----- > Add Custom Menus
        self.stackedWidget.setMinimumWidth(20)
        UIF.addNewMenu(self, "Perturbation", "btn_perturbation",
                       "url(:/20x20/icons/20x20/chemical_20x20.png)", True)
        UIF.addNewMenu(self, "Monitoring", "btn_monitoring",
                       "url(:/20x20/icons/20x20/cil-monitor.png)", True)
        UIF.addNewMenu(self, "Analysis", "btn_analysis",
                       "url(:/16x16/icons/16x16/cil-chart-line.png)", True)
        """
        UIF.addNewMenu(self, "Settings", "btn_settings",
                                   "url(:/20x20/icons/20x20/cil-equalizer.png)", False)
        """
        UIF.addNewMenu(self, "About & Contact", "btn_about", "url(:/20x20/icons/20x20/cil-tag.png)", False)

        # ----- > Start Menu Selection
        UIF.selectStandardMenu(self, "btn_perturbation")

        # ----- > Start Page
        self.stackedWidget.setCurrentWidget(self.page_home)

        # ----- > Show / Hide User Icon
        UIF.userIcon(self, "HIO", "", True)

        # self.frame_label_top_btns.mouseMoveEvent = self.moveWindow

        # ----- > Load Definitions
        UIF.uiDefinitions(self)
        ...
        self.quit_pushButton.clicked.connect(lambda: UIF.close_application(self))
        ...
        self.mutator_checkBox.stateChanged.connect(lambda: Functions.mutator_isActive(self))
        ...
        self.equilubrate_checkBox.stateChanged.connect(lambda: Functions.Equilibration_On_Off_Changed(self))
        self.Device_ID_checkBox.stateChanged.connect(lambda: Functions.UseDeviceID_On_Off_Changed(self))
        ...
        self.load_sim_sample_pushButton.clicked.connect(lambda: Functions.load_sample_for_simulation(self))
        self.export_workspace_pushButton.clicked.connect(lambda: Functions.export_workspace(self))
        self.import_workspace_pushButton.clicked.connect(lambda: Functions.import_workspace(self))
        ...
        self.Number_of_steps_spinBox.valueChanged.connect(
            lambda: Functions.number_of_steps_changed_from_advanced(self))
        ...
        self.All_CPU_checkBox.stateChanged.connect(lambda: Functions.All_CPU_Usage_State(self))
        ...
        self.load_analysis_sample_pushButton.clicked.connect(lambda: Functions.load_sample_for_analysis(self))

        self.network_calculate_pushButton.clicked.connect(self.run_network_analysis)

        # ----------------------------------- > START OF PYMOL RELEATED BUTTONS < ------------------------------------ #
        UIF.start_pymol(self)
        ...
        self.selected_residues_listWidget.itemDoubleClicked.connect(lambda: UIF.show_residue_labels(self))
        self.refresh_pushButton.clicked.connect(lambda: UIF.clear_residue_labels(self))
        self.activate_pymol_navigation.clicked.connect(lambda: UIF.activate_navigation_on_Pymol(self))
        self.deactivate_pymol_navigation.clicked.connect(lambda: UIF.deactivate_navigation_on_Pymol(self))
        self.visualization_Handel_buttons_changing()
        self.Handel_Buttons()
        self.ss_beatiful_snapshoot.clicked.connect(lambda: UIF.show_beatiful_in_Pymol(self))
        self.get_figure_pushButton.clicked.connect(lambda: UIF.save_as_png_Pymol(self))
    ...
    def show_simulation_monitoring(self):
        ...
        UIF.resetStyle(self, "btn_monitoring")
        UIF.labelPage(self, "MONITORING THE PROCESS")
        widget = self.findChild(QPushButton, "btn_monitoring")
        widget.setStyleSheet(UIF.selectMenu(widget.styleSheet()))
    ...
    def fetch_and_load_pdbfile(self):
        ...
        if fetched_and_modified_pdb:
            UIF.load_pdb_to_pymol(self, fetched_and_modified_pdb)
        ...
        try:
            ...
            if os.path.exists(pdb_path):
                ...
                if pdb_fix_dialog_answer == QtWidgets.QDialog.Accepted:
                    ...
                    modified_pdb = pdb_Tools.fetched_pdb_fix(self, pdb_path,
                ...
                UIF.load_pdb_to_pymol(self, modified_pdb)
    ...
    def upload_boundForm_pdb_from_local(self, manuel=False):
        ...
        try:
            ...
            if os.path.exists(pdb_path):
                ...
                UIF.load_pdb_to_3DNetwork(self, modified_pdb)
    ...
    def Button(self):
        ...
        if btnWidget.objectName() == "btn_home":
            self.stackedWidget.setCurrentWidget(self.page_home)
            UIF.resetStyle(self, "btn_home")
            UIF.labelPage(self, "Home")
            btnWidget.setStyleSheet(UIF.selectMenu(btnWidget.styleSheet()))

        # PAGE PERTURBATION
        if btnWidget.objectName() == "btn_perturbation":
            self.stackedWidget.setCurrentWidget(self.page_home)
            UIF.resetStyle(self, "btn_perturbation")
            UIF.labelPage(self, "Perturbation User Interface")
            btnWidget.setStyleSheet(UIF.selectMenu(btnWidget.styleSheet()))
        ...
        if btnWidget.objectName() == "btn_analysis":
            ...
            UIF.resetStyle(self, "btn_analysis")
            UIF.labelPage(self, "ENERGY DISSIPATION ANALYSIS")
            btnWidget.setStyleSheet(UIF.selectMenu(btnWidget.styleSheet()))

        # PAGE WIDGETS
        if btnWidget.objectName() == "btn_settings":
            self.stackedWidget.setCurrentWidget(self.page_settings)
            UIF.resetStyle(self, "btn_settings")
            UIF.labelPage(self, "Settings")
            btnWidget.setStyleSheet(UIF.selectMenu(btnWidget.styleSheet()))

        # PAGE ABOUT
        if btnWidget.objectName() == "btn_about":
            self.stackedWidget.setCurrentWidget(self.page_contact)
            UIF.resetStyle(self, "btn_about")
            UIF.labelPage(self, "ABOUT & CONTACT")
            btnWidget.setStyleSheet(UIF.selectMenu(btnWidget.styleSheet()))
    ...
    def moveWindow(self, event):
        ...
        if UIF.returStatus() == 1:
            UIF.maximize_restore(self)
        # ----- > Move Window
